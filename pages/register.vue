...
const state = reactive({ name: '', email: '', password: '', passwordConfirmation: '' })
...
async function onSubmit() {
  pending.value = true
  const res = await register(state.name, state.email, state.password, state.passwordConfirmation)
  pending.value = false
  ...
  <div class="min-h-screen flex items-center justify-center bg-neutral-50 px-4">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8">
        ...
        <p class="mt-2 text-sm text-neutral-500">Create your account</p>
      </div>

      <!-- Card -->
      <div class="bg-white rounded-xl border border-neutral-200 shadow-sm p-8 space-y-6">
        ...
        <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
          ...
          <UButton type="submit" block :loading="pending" color="neutral" variant="solid">
            Create account
          </UButton>
        </UForm>
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          >
            <UIcon name="i-simple-icons-google" class="w-4 h-4 text-[#4285F4]" />
            Google
          </button>
          <button
        ...
        <p class="text-center text-sm text-neutral-500">
          ...
          <NuxtLink to="/login" class="font-medium text-neutral-900 hover:underline">
            Sign in
          </NuxtLink>
        </p>
